...
type Employee = { id: string; full_name: string; role: string; joining_date: string | null };
...
// Monthly report helpers — only Sunday is non-working (Saturday is a full working day)
...
function getCycle(
  ...
  if (!joiningDate) {
    ...
    return { start, end: `${refYear}-${String(refMonth).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}` };
  }
...
export default function AdminPanelPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: emps }, { data: settings }] = await Promise.all([
        supabase.from("employees").select("id, full_name, role, joining_date").eq("is_active", true).order("full_name"),
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: att }, { data: lv }] = await Promise.all([
        supabase.from("attendance").select("id, employee_id, date, check_in, check_out")
          .gte("date", wideStart).lte("date", wideEnd),
        supabase.from("leaves").select("id, employee_id, start_date, end_date, status")
          .eq("status", "approved"),
      ]);
      setSummaryAtt(att ?? []);
      setApprovedLeaves(lv ?? []);
    })();
  }, [authorized, cycleYear, cycleMonth]);
  ...
  // ── Computed: monthly report summaries ───────────────────────────────────────
  ...
  // Export CSV — monthly report data
  const exportMonthlyCSV = () => {
    const headers = [
      "Employee Name", "Role", "Cycle Start", "Cycle End",
      "Working Days", "Present Days", "Absent Days", "Leave Days",
      "Total Hours Worked", "Overtime Hours", "Absent Dates",
    ];
    ...
    const fileStart = first?.cycleStart ?? `${cycleYear}-${String(cycleMonth).padStart(2, "0")}-01`;
    const fileEnd = first?.cycleEnd ?? `${cycleYear}-${String(cycleMonth).padStart(2, "0")}-30`;
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
  ...
  const handleExportCSV = () => {
    if (activeTab === "monthly_report") exportMonthlyCSV();
    else exportDailyCSV();
  };

  // Cycle navigation
  const prevCycle = () => {
    if (cycleMonth === 1) { setCycleYear((y) => y - 1); setCycleMonth(12); }
    else setCycleMonth((m) => m - 1);
  };
  const nextCycle = () => {
    const isCurrent = cycleYear === istNow.getFullYear() && cycleMonth === istNow.getMonth() + 1;
    if (isCurrent) return;
    if (cycleMonth === 12) { setCycleYear((y) => y + 1); setCycleMonth(1); }
    else setCycleMonth((m) => m + 1);
  };
  const isCycleCurrent = cycleYear === istNow.getFullYear() && cycleMonth === istNow.getMonth() + 1;
  ...
  return (
    <div className="text-white">
        ...
        <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded-lg p-1 shrink-0">
          <button
            ...
            className={`text-xs font-semibold px-3 py-1.5 rounded-md transition-colors ${
              activeTab === "daily" ? "bg-zinc-700 text-white" : "text-zinc-500 hover:text-zinc-300"
            }`}
          >
            Today&apos;s Attendance
          </button>
          <button
            ...
            className={`text-xs font-semibold px-3 py-1.5 rounded-md transition-colors ${
              activeTab === "monthly_report" ? "bg-zinc-700 text-white" : "text-zinc-500 hover:text-zinc-300"
            }`}
          >
            Monthly Report
          </button>
      ...
      {/* ══════════════════════════════════════════════════════
          TAB: TODAY'S ATTENDANCE (existing)
      ══════════════════════════════════════════════════════ */}
      {activeTab === "daily" && (
                          ...
                          return (
                            <th
                              ...
                              className={`px-1 py-2.5 font-medium text-center w-9 ${
                                isToday ? "text-white" : weekend ? "text-zinc-700" : "text-zinc-500"
                              }`}
                            >
                              <div>{day}</div>
      ...
      {/* ══════════════════════════════════════════════════════
          TAB: MONTHLY REPORT (new)
      ══════════════════════════════════════════════════════ */}
      {activeTab === "monthly_report" && (
        <div className="px-4 py-6 md:px-8 pb-20 space-y-6">
          ...
          <p className="text-zinc-600 text-xs">
            Working days exclude Sundays only. Saturday counts as a full working day.
          </p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm min-w-[900px]">
                  <thead>
                    <tr className="border-b border-zinc-800 bg-zinc-900">
                      <th className="text-left px-4 py-3 text-zinc-500 font-medium">Employee</th>
                      <th className="text-left px-4 py-3 text-zinc-500 font-medium">Cycle Period</th>
                      <th className="text-center px-4 py-3 text-zinc-500 font-medium">Working Days</th>
                      <th className="text-center px-4 py-3 text-zinc-500 font-medium">Present</th>
                      <th className="text-center px-4 py-3 text-zinc-500 font-medium">Absent</th>
                      <th className="text-center px-4 py-3 text-zinc-500 font-medium">On Leave</th>
                      <th className="text-center px-4 py-3 text-zinc-500 font-medium">Total Hours</th>
                      <th className="text-center px-4 py-3 text-zinc-500 font-medium">Overtime</th>
                      <th className="text-left px-4 py-3 text-zinc-500 font-medium">Absent Dates</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-800/60">
                    {summaries.flatMap((s) => {
                      const isExpanded = expandedEmployee === s.employee.id;
                      const hasDetail = s.absentDates.length > 0 || s.leaveDates.length > 0;
                      ...
                      const rows = [
                        ...
                        >
                          <td className="px-4 py-3">
                            <p className="text-white font-semibold whitespace-nowrap">{s.employee.full_name}</p>
                            <p className="text-zinc-500 text-xs">{s.employee.role}</p>
                          </td>
                          <td className="px-4 py-3 text-zinc-400 text-xs whitespace-nowrap">
                            {fmtDateShort(s.cycleStart)} → {fmtDateShort(s.cycleEnd)}
                          </td>
                          <td className="px-4 py-3 text-center text-zinc-300">{s.workingDays}</td>
                          <td className="px-4 py-3 text-center text-emerald-400 font-semibold">{s.presentDays}</td>
                          <td className="px-4 py-3 text-center">
                            <span className={s.absentDays > 0 ? "text-red-400 font-semibold" : "text-zinc-500"}>
                              {s.absentDays}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center text-blue-400">{s.leaveDays}</td>
                          <td className="px-4 py-3 text-center font-mono text-xs text-zinc-300">
                            {fmtHHMM(s.totalMinutes)}
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span className={s.overtimeMinutes > 0 ? "text-amber-400 font-mono text-xs" : "text-zinc-600 text-xs"}>
                          ...
                          <td className="px-4 py-3">
                            {hasDetail ? (
                              <button
                                onClick={() => setExpandedEmployee(isExpanded ? null : s.employee.id)}
                                className="text-xs text-zinc-400 hover:text-white underline underline-offset-2 transition-colors whitespace-nowrap"
                              >
                                {isExpanded ? "Hide" : `Show ${s.absentDates.length} date${s.absentDates.length !== 1 ? "s" : ""}`}
                              </button>
                            ) : (
